...
class Menu 
  ...
  def set_route
    ...
    puts "Поезду #{@trains[@num_train].name} назначен маршрут"
  end
  ...
  def  train_type_passenger?
    if @trains[@num_train].train_type == :passenger
    end
  end 
  ...
  def choose_train
    ...
    puts "Поезд #{@trains[@num_train].name} - #{@trains[@num_train].train_type}"
  end

  def all_trains_list
    @trains.each_with_index {|train, index| puts "#{index} - <<#{train.name}>>"}
  end
